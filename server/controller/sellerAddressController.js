...
const { SellerAddress, Seller } = require("../../models");
...
const getOneUserAllAddress = async (req, res) => {
    ...
    SellerAddress.findAll({
        ...
        where: { [Op.and]: [Active, Deleted, { SellerId: id }] },
        order: [["id", "DESC"]],
    })
        .then((data) => {
            res.json(data);
...
const getOneAddress = async (req, res) => {
    const { id } = req.params;
    const data = await SellerAddress.findOne({ where: { id: id } });
    if (data) {
...
const create = async (req, res) => {
    const { welayat, etrap, address, lat, lang, SellerId } = req.body;
    const data = await Seller.findOne({ where: { id: SellerId } });
...
const disActive = async (req, res) => {
    const { id } = req.params;
    let data = await SellerAddress.findOne({ where: { id: id } });
    if (data) {
        SellerAddress.update(
            {
                active: false,

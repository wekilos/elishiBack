...
const getOneSeller = async (req, res) => {
    const { id } = req.params;
    const user = await Seller.findOne({ where: { id: id } });
    if (user) {
        Seller.findOne({
            where: {
                id: id,
...
const create = async (req, res) => {
    ...
    let fileDirection = "";
    const existUser = await Seller.findOne({
        where: {
            phone: phone,
        },
    });
...
const forgot = async (req, res) => {
    ...
    const user = await Seller.findOne({ where: { phone: phone } });
    if (!user) {
...
const disActive = async (req, res) => {
    const { id } = req.params;
    let user = await Seller.findOne({ where: { id } });
    if (user) {
        Seller.update(
            {
                active: false,

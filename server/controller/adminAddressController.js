...
const { AdminAddress, Admin } = require("../../models");
...
const getOneUserAllAddress = async (req, res) => {
    ...
    AdminAddress.findAll({
        ...
        where: { [Op.and]: [Active, Deleted, { AdminId: id }] },
        order: [["id", "DESC"]],
    })
        .then((data) => {
            res.json(data);
...
const getOneAddress = async (req, res) => {
    const { id } = req.params;
    const data = await AdminAddress.findOne({ where: { id: id } });
    if (data) {
        AdminAddress.findOne({
            where: {
                id: id,
            },
        })
...
const create = async (req, res) => {
    const { welayat, etrap, address, lat, lang, AdminId } = req.body;
    const data = await Admin.findOne({ where: { id: AdminId } });
...
const disActive = async (req, res) => {
    const { id } = req.params;
    let data = await AdminAddress.findOne({ where: { id: id } });
    if (data) {
        AdminAddress.update(
            {
                active: false,
